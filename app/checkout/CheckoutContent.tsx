...
function Field({ label, children }: { label: string; children: React.ReactNode }) {
  return (
    <div className="flex flex-col gap-1.5">
      <label className="text-[11px] tracking-wider uppercase" style={{ color: "rgba(168,50,74,0.6)" }}>
        {label}
      </label>
      {children}
    </div>
...
function Step({ number, title, children }: { number: number; title: string; children: React.ReactNode }) {
  return (
    <div className="rounded-2xl overflow-hidden" style={{ background: "#FFFFFF", border: "1px solid rgba(168,50,74,0.14)" }}>
      <div className="flex items-center gap-3 px-4 py-4" style={{ borderBottom: "1px solid rgba(168,50,74,0.1)" }}>
        <span className="w-7 h-7 rounded-full flex items-center justify-center text-xs font-bold shrink-0"
          style={{ background: "rgba(168,50,74,0.15)", border: "1px solid rgba(168,50,74,0.4)", color: "#A8324A" }}>
          {number}
        </span>
        <span className="font-serif text-sm font-semibold" style={{ color: "#2B2220" }}>{title}</span>
      </div>
...
function SaveOccasionBlock({ customerPhone }: { customerPhone: string }) {
  ...
  if (saved) return (
    <div className="flex items-center gap-2 rounded-xl px-4 py-3 text-xs"
      style={{ background: "rgba(168,50,74,0.08)", border: "1px solid rgba(168,50,74,0.25)", color: "#A8324A" }}>
  ...
  return (
    <div className="rounded-xl overflow-hidden" style={{ border: "1px solid rgba(168,50,74,0.12)" }}>
      <button type="button" onClick={() => setOpen(o => !o)}
        className="w-full flex items-center justify-between px-4 py-3 text-xs text-left transition-colors"
        style={{ background: "rgba(168,50,74,0.05)", color: "rgba(110,95,89,0.55)" }}>
        <span>🎂 Запомни именния ден на близък човек</span>
        <span style={{ color: "rgba(168,50,74,0.6)" }}>{open ? "−" : "+"}</span>
      </button>
      {open && (
        <div className="px-4 pb-4 pt-3 flex flex-col gap-3" style={{ borderTop: "1px solid rgba(168,50,74,0.1)" }}>
          ...
          <button type="button" onClick={handleSave}
            ...
            style={{ background: "rgba(168,50,74,0.15)", border: "1px solid rgba(168,50,74,0.3)", color: "#A8324A" }}>
...
export function CheckoutContent() {
  ...
  return (
    <div className="min-h-screen flex flex-col pb-36" style={{ background: "#FBF6F3" }}>
      ...
      <header className="sticky top-0 z-40 flex items-center gap-3 px-4 py-4"
        style={{ background: "rgba(251,246,243,0.9)", backdropFilter: "blur(16px)", borderBottom: "1px solid rgba(168,50,74,0.1)" }}>
        <Link href="/" className="p-1.5 rounded-lg hover:bg-black/5 transition-colors">
          <ArrowLeft className="w-4 h-4" style={{ color: "#A8324A" }} />
        </Link>
        <span className="font-serif text-lg font-semibold text-primary">Kiss My Flowers</span>
        <span className="text-xs" style={{ color: "rgba(110,95,89,0.55)" }}>/ Поръчка</span>
      </header>
      ...
      <div className="mx-3 mt-3 rounded-2xl overflow-hidden"
        ...
        <button type="button" onClick={() => setSummaryOpen(o => !o)}
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-[10px] mt-0.5" style={{ color: "rgba(110,95,89,0.4)" }}>
              {product.flower_count} стъбла · + доставка{upsellTotal > 0 ? ` · + добавки` : ""}
            </p>
          </div>
          <div className="flex items-center gap-2 shrink-0">
            <DualPrice priceEur={totalEur} layout="inline" />
            {summaryOpen ? <ChevronUp className="w-3.5 h-3.5" style={{ color: "rgba(168,50,74,0.5)" }} />
              : <ChevronDown className="w-3.5 h-3.5" style={{ color: "rgba(168,50,74,0.5)" }} />}
          </div>
        </button>
        {summaryOpen && (
          <div className="px-4 pb-4 flex flex-col gap-2 text-xs" style={{ borderTop: "1px solid rgba(168,50,74,0.1)" }}>
            <div className="flex justify-between pt-3" style={{ color: "rgba(110,95,89,0.5)" }}>
              <span>{product.title}</span><DualPrice priceEur={product.price_eur} layout="inline" />
            </div>
            {UPSELLS.filter(u => upsells[u.id]).map(u => (
              <div key={u.id} className="flex justify-between" style={{ color: "rgba(110,95,89,0.5)" }}>
                <span>{u.emoji} {u.title}</span><DualPrice priceEur={u.price_eur} layout="inline" />
              </div>
            ))}
            <div className="flex justify-between" style={{ color: "rgba(110,95,89,0.5)" }}>
              <span>Доставка</span><DualPrice priceEur={DELIVERY_FEE_EUR} layout="inline" />
            </div>
            <div className="flex justify-between font-semibold pt-2"
              style={{ borderTop: "1px solid rgba(168,50,74,0.1)", color: "#A8324A" }}>
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-3 px-3 mt-3">
          ...
          <div className="flex flex-col gap-4">
            <Field label="Телефон (Viber / SMS) *">
              <div className="flex items-stretch rounded-[10px] overflow-hidden"
                style={{ border: "1px solid rgba(168,50,74,0.18)", background: "#FFFFFF" }}>
                <div className="flex items-center gap-1.5 px-3"
                  style={{ borderRight: "1px solid rgba(168,50,74,0.15)", background: "rgba(168,50,74,0.06)" }}>
                  <span>🇧🇬</span>
                  <span className="text-xs font-medium" style={{ color: "rgba(168,50,74,0.8)" }}>+359</span>
                </div>
                <input required type="tel" className="flex-1 bg-transparent px-3 py-3 text-sm outline-none"
                  style={{ color: "#2B2220" }} placeholder="88 888 8888"
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="grid grid-cols-2 gap-2">
              {(["recipient", "self"] as const).map(v => (
                <button key={v} type="button" onClick={() => setDeliveryFor(v)}
                  ...
                  style={deliveryFor === v
                    ? { background: "rgba(168,50,74,0.15)", border: "1px solid rgba(168,50,74,0.5)", color: "#A8324A" }
                    : { background: "transparent", border: "1px solid rgba(168,50,74,0.14)", color: "rgba(110,95,89,0.45)" }}>
            ...
            {deliveryFor === "recipient" && (
              <>
                ...
                <div className="flex items-start gap-2.5 rounded-xl px-3 py-3"
                  style={{ background: "rgba(168,50,74,0.06)", border: "1px solid rgba(168,50,74,0.14)" }}>
                  <EyeOff className="w-4 h-4 shrink-0 mt-0.5" style={{ color: "rgba(168,50,74,0.7)" }} />
                  <p className="text-[11px] leading-relaxed" style={{ color: "rgba(110,95,89,0.55)" }}>
                    Доставката е <strong style={{ color: "rgba(168,50,74,0.9)" }}>анонимна</strong> — получателят не разбира кой е изпратил, освен ако посочите себе си в съобщението.
                  </p>
                </div>
              </>
            )}
            ...
            <div className="flex flex-col gap-2">
              ...
              <div className="flex items-center justify-between">
                <span className="text-[10px]" style={{ color: "rgba(110,95,89,0.3)" }}>
                  {greetingMessage.length}/300
                </span>
                <span className="text-[10px]" style={{ color: "rgba(168,50,74,0.5)" }}>
                  Готови шаблони →
                </span>
              </div>
              {/* Templates */}
              <div className="flex flex-col gap-1.5">
                {GREETING_TEMPLATES.map(t => (
                  <button key={t} type="button" onClick={() => setGreetingMessage(t)}
                    ...
                    style={{
                      background: greetingMessage === t ? "rgba(168,50,74,0.12)" : "#FFFFFF",
                      border: `1px solid ${greetingMessage === t ? "rgba(168,50,74,0.3)" : "#ECDFD9"}`,
                      color: greetingMessage === t ? "#A8324A" : "rgba(110,95,89,0.4)",
                    }}>
                    {t}
                  </button>
                ))}
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="grid grid-cols-2 gap-2">
              {([
                ...
                <button key={v} type="button" onClick={() => setSchedule(v)}
                  ...
                  style={schedule === v
                    ? { background: "rgba(168,50,74,0.12)", border: "1px solid rgba(168,50,74,0.45)" }
                    : { background: "transparent", border: "1px solid rgba(168,50,74,0.12)" }}>
                  <p className="text-xs font-semibold" style={{ color: schedule === v ? "#A8324A" : "#2B2220" }}>
                    {label}
                  </p>
                  <p className="text-[10px] mt-0.5" style={{ color: "rgba(110,95,89,0.4)" }}>{sub}</p>
                </button>
              ))}
            </div>
            ...
            {schedule === "asap" && (
              <div className="flex items-center gap-2 rounded-xl px-3 py-3"
                style={{ background: "rgba(168,50,74,0.06)", border: "1px solid rgba(168,50,74,0.14)" }}>
                <Clock className="w-4 h-4 shrink-0" style={{ color: "rgba(168,50,74,0.7)" }} />
                <p className="text-[11px]" style={{ color: "rgba(110,95,89,0.55)" }}>
                  Доставка до <strong style={{ color: "#A8324A" }}>2 часа</strong> след потвърждение.
              ...
              <div className="flex flex-col gap-3">
                <Field label="Дата на доставка">
                  <div className="relative">
                    <Calendar className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 pointer-events-none" style={{ color: "rgba(168,50,74,0.5)" }} />
                    <input type="date" min={today}
                      value={deliveryDate} onChange={e => setDeliveryDate(e.target.value)}
                      className="w-full rounded-[10px] pl-9 pr-3 py-3 text-sm outline-none"
                      style={{ ...S, colorScheme: "light" }} />
                  </div>
                </Field>
                  ...
                  <div className="grid grid-cols-2 gap-2 sm:grid-cols-3">
                    {TIME_WINDOWS.map(w => (
                      <button key={w} type="button" onClick={() => setTimeWindow(w)}
                        ...
                        style={timeWindow === w
                          ? { background: "rgba(168,50,74,0.15)", border: "1px solid rgba(168,50,74,0.5)", color: "#A8324A" }
                          : { background: "#FFFFFF", border: "1px solid rgba(168,50,74,0.12)", color: "rgba(110,95,89,0.5)" }}>
                        {w}
                      </button>
                    ))}
                  </div>
        ...
        {/* ── Upsells ── */}
        <div className="rounded-2xl overflow-hidden" style={{ background: "#FFFFFF", border: "1px solid rgba(168,50,74,0.14)" }}>
          <div className="px-4 py-3.5" style={{ borderBottom: "1px solid rgba(168,50,74,0.1)" }}>
            <p className="text-sm font-serif font-semibold" style={{ color: "#2B2220" }}>Добавете към букета</p>
            <p className="text-[11px] mt-0.5" style={{ color: "rgba(110,95,89,0.38)" }}>Завършете подаръка с нещо специално</p>
          </div>
            ...
            {UPSELLS.map((u, i) => {
              ...
              return (
                <div key={u.id} className="flex items-center gap-3 px-4 py-3.5"
                  style={{ borderTop: i > 0 ? "1px solid rgba(168,50,74,0.07)" : undefined }}>
                  <span style={{ fontSize: 28 }}>{u.emoji}</span>
                  <div className="flex-1 min-w-0">
                    <p className="text-sm" style={{ color: "#2B2220" }}>{u.title}</p>
                  ...
                  <button type="button" onClick={() => setUpsells(s => ({ ...s, [u.id]: !s[u.id] }))}
                    ...
                    style={selected
                      ? { background: "#A8324A", color: "#FFFFFF" }
                      : { background: "rgba(168,50,74,0.1)", border: "1px solid rgba(168,50,74,0.3)", color: "#A8324A" }}>
        ...
        <Step number={5} title="Плащане">
          <div className="flex flex-col gap-3">
            ...
            <div className="flex items-start gap-3 rounded-xl p-4"
              style={{ background: "rgba(168,50,74,0.07)", border: "1px solid rgba(168,50,74,0.28)" }}>
              <CreditCard className="w-5 h-5 shrink-0 mt-0.5" style={{ color: "#A8324A" }} />
              <div>
                <p className="text-sm font-semibold" style={{ color: "#2B2220" }}>
                  Карта · Apple Pay · Google Pay
                </p>
                <p className="text-[11px] mt-1 leading-relaxed" style={{ color: "rgba(110,95,89,0.45)" }}>
            ...
            <div className="flex items-center justify-center gap-4 pt-1">
              <span className="flex items-center gap-1.5 text-[10px]" style={{ color: "rgba(110,95,89,0.28)" }}>
                <Lock className="w-3 h-3" /> SSL 256-bit
              </span>
              <span className="text-[10px]" style={{ color: "rgba(168,50,74,0.2)" }}>·</span>
              <span className="text-[10px]" style={{ color: "rgba(110,95,89,0.28)" }}>Powered by Stripe</span>
              <span className="text-[10px]" style={{ color: "rgba(168,50,74,0.2)" }}>·</span>
              <span className="text-[10px]" style={{ color: "rgba(110,95,89,0.28)" }}>3D Secure</span>
            </div>
            {/* GDPR / legal consent */}
            <p className="text-[10px] text-center leading-relaxed" style={{ color: "rgba(110,95,89,0.3)" }}>
              Натискайки „Продължи към плащане", приемате{" "}
              <a href="/terms" target="_blank" style={{ color: "rgba(168,50,74,0.6)", textDecoration: "underline" }}>Условията за ползване</a>,{" "}
              <a href="/refund" target="_blank" style={{ color: "rgba(168,50,74,0.6)", textDecoration: "underline" }}>Политиката за връщане</a>{" "}
              и обработката на лични данни съгласно{" "}
              <a href="/privacy" target="_blank" style={{ color: "rgba(168,50,74,0.6)", textDecoration: "underline" }}>Политиката за поверителност</a> (GDPR).
            </p>
          </div>
        </Step>
        ...
        <div className="rounded-xl overflow-hidden" style={{ border: "1px solid rgba(168,50,74,0.12)" }}>
          <button type="button" onClick={() => setPromoOpen(o => !o)}
            ...
            style={{ background: "rgba(168,50,74,0.04)", color: "rgba(110,95,89,0.5)" }}>
            <Tag className="w-3.5 h-3.5" style={{ color: "rgba(168,50,74,0.6)" }} />
            <span>Имам промокод</span>
            <span className="ml-auto" style={{ color: "rgba(168,50,74,0.5)" }}>{promoOpen ? "−" : "+"}</span>
          </button>
          {promoOpen && (
            <div className="px-4 pb-4 pt-3 flex gap-2" style={{ borderTop: "1px solid rgba(168,50,74,0.1)" }}>
            ...
            {[
              ...
              <label key={id} className="flex items-start gap-3 cursor-pointer">
                <div className="w-5 h-5 rounded-md mt-0.5 shrink-0 flex items-center justify-center transition-all"
                  style={{ background: checked ? "#A8324A" : "transparent", border: `1.5px solid ${checked ? "#A8324A" : "rgba(168,50,74,0.3)"}` }}
                ...
                <span className="text-xs leading-relaxed" style={{ color: "rgba(110,95,89,0.5)" }}>{label}</span>
              </label>
            ))}
        ...
        {error && (
          <div className="flex items-start gap-2 p-4 rounded-xl text-xs"
            style={{ background: "rgba(179,38,30,0.08)", border: "1px solid rgba(179,38,30,0.3)", color: "#B3261E" }}>
            <AlertTriangle className="w-4 h-4 shrink-0 mt-0.5 text-red-400" />
            <span>{error}</span>
          </div>
        )}

        <p className="text-center text-[10px] px-2" style={{ color: "rgba(110,95,89,0.2)" }}>
      ...
      <div className="fixed bottom-0 left-0 right-0 z-40 px-4 py-4"
        style={{ background: "rgba(255,255,255,0.97)", backdropFilter: "blur(20px)", borderTop: "1px solid rgba(168,50,74,0.12)", paddingBottom: "calc(env(safe-area-inset-bottom, 0px) + 16px)" }}>
        <div className="flex items-center justify-between mb-3">
          <div className="flex flex-col gap-0.5">
            <span className="text-[10px]" style={{ color: "rgba(110,95,89,0.35)" }}>
        ...
        <button type="submit" form="" onClick={handleSubmit as unknown as React.MouseEventHandler}
          ...
          style={{ background: "linear-gradient(135deg, #C24B5E 0%, #8E2438 100%)", color: "#FFFFFF", boxShadow: "0 6px 20px rgba(168,50,74,0.3)" }}>
          {loading
