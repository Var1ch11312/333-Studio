...
| Медиа | WebRTC P2P, audio-only + TURN | P2P как в ТЗ; TURN обязателен (см. §8.2) |
...
| SEO | Next Metadata API + route groups | Граница индексации объявляется один раз на группу, а не тегом на страницу |
...
│   ├── robots.ts                         # SEO: allow crawl, точечный disallow
│   ├── sitemap.ts                        # SEO: только публичные 200/self-canonical URL
│   ├── opengraph-image.tsx  icon.tsx     # SEO: OG-картинка и фавиконка
│   │
│   ├── (marketing)/                      # ── ПУБЛИЧНО, ИНДЕКСИРУЕТСЯ ──
│   │   ├── layout.tsx                    #    metadataBase, canonical, hreflang, JSON-LD
│   │   ├── page.tsx                      #    лендинг: как работает цепочка
│   │   ├── how-it-works/page.tsx
│   │   ├── faq/page.tsx                  #    FAQPage JSON-LD (только видимые Q&A)
│   │   └── (legal)/{terms,privacy}/page.tsx
│   │
│   ├── (auth)/                           # ── ПУБЛИЧНО, НО noindex ──
│   │   ├── layout.tsx                    #    robots: { index: false }
│   │   ├── login/page.tsx                #    ЭКРАН 1 — вход (email → magic link)
│   │   └── auth/
│   │       ├── verify/page.tsx           #    кнопка подтверждения (защита от префетча почтовиков)
│   │       └── callback/route.ts         #    обмен кода на сессию
│   │
│   ├── (app)/                            # ── ПРИВАТНО, НИКОГДА НЕ ИНДЕКСИРУЕТСЯ ──
...
│   └── seo/         JsonLd Breadcrumbs                    # server components
│
├── lib/
...
│   ├── seo.ts               # metadataBase, hreflang-матрица, JSON-LD билдеры  ⚠
...
WakeChain — приложение за логином. Индексируемой поверхности у него почти нет: `/home`, `/call/[id]`, `/profile` не должны попадать в выдачу **никогда**, и это требование приватности, а не только SEO. Поэтому «SEO на всех этапах» распадается на две разные работы:

- **Привлечение** — публичный маркетинговый слой (`(marketing)/`). Его в изначальном ТЗ не было; это добавленный scope, и он реальный: лендинг, «как это работает», FAQ, юридические страницы.
- **Гигиена индексации** — на всех остальных этапах. Не «оптимизировать под запрос», а не утечь в индекс и не сломать рендер краулеру.

Ниже расписано и то, и другое. Там, где у этапа нет поисковой поверхности, так и написано — вместо выдуманных задач.

### 7.2 Граница индексации

```
(marketing)/  → индексируется   полная метадата, canonical, hreflang, JSON-LD, в sitemap
(auth)/       → noindex         краулится, но не индексируется; НЕ в sitemap
(app)/        → noindex + auth  ни одной ссылки с публичных страниц
```

Одна декларация на группу, а не тег на каждой странице:
...
**Ключевая ловушка:** `Disallow` в `robots.txt` — это запрет **обхода**, а не индексации. Закрытая через `Disallow` страница всё равно может появиться в выдаче голым URL — потому что краулеру запретили прочитать наш же `noindex`. Приватные маршруты обходить **разрешаем**, отдаём `noindex`, а реально защищаем авторизацией.
...
### 7.3 Язык и письменность

Зависит от вопроса 8.3, но правило одно и оно дороже всей остальной технички: **писать в той письменности, в которой люди ищут.** Болгарин набирает `будилник`, не `budilnik`. Транслитерированный латиницей заголовок не совпадает ни с одним запросом.

Это не гипотетическая ошибка — в соседнем AMUR.BG сейчас именно так (`"AMUR.BG — Dostavka na tsveta v Burgas"`, `keywords: ["tsvetya", …]`). Чинить его не в рамках этой задачи, но в WakeChain повторять не будем.

`keywords` не ставим вообще — поисковики не используют этот тег с 2009 года.

### 7.4 hreflang

Если языков больше одного — матрица обязана быть взаимной и самоссылающейся, иначе игнорируется целиком:

```ts
alternates: {
  canonical: `${BASE}/ru/how-it-works`,
  languages: {
    ru: `${BASE}/ru/how-it-works`,
    bg: `${BASE}/bg/how-it-works`,
    en: `${BASE}/en/how-it-works`,
    'x-default': `${BASE}/en/how-it-works`,
  },
}
```

### 7.5 Service worker против краулера

Самый дорогой баг в связке PWA + SEO: SPA-фолбэк отдаёт закешированную оболочку на **любую** навигацию. Googlebot запрашивает `/how-it-works`, получает пустой шелл и индексирует пустую страницу.

```js
// public/sw.js
if (e.request.mode === 'navigate') return   // документы — только из сети
// кешируем ТОЛЬКО хешированные статические ассеты
```

Проверять в URL Inspection по **отрендеренному HTML**, не по «просмотру исходного кода».

### 7.6 Что проверяется на каждом этапе

| # | Срез | Работа по SEO |
|---|---|---|
| 1 | Скелет | `metadataBase`, `robots.ts`, `sitemap.ts`, route-группы `(marketing)/(auth)/(app)` заведены сразу — задним числом это рефактор роутинга |
| 2 | Magic link | `(auth)` под `noindex`. Токен не должен утечь в `Referer` и в аналитику |
| 3 | Онбординг | Поисковой поверхности нет. Только проверка, что `(app)` унаследовал `noindex` |
| 4 | Матчинг / крон | Поверхности нет. `/api/` в `Disallow` |
| 5 | Пуш + PWA | **Критично:** SW не отдаёт шелл на навигации; HTML без долгого `max-age`; `manifest.ts` согласован с метадатой |
| 6 | Звонок | Поверхности нет — и не должно быть. Убедиться, что `/call/[id]` нет ни в sitemap, ни в ссылках |
| 7 | Эстафета | То же. `noindex` на `(app)` покрывает |
| 8 | Эскалация | Поверхности нет |
| 9 | Профиль | `noindex`. Публичных профилей в v1 нет — не добавлять «для SEO», это утечка приватности |
| 10 | Безопасность | Страницы жалоб и блокировок — `noindex` |
| **11** | **Маркетинговый слой** | **Новый срез:** лендинг, how-it-works, FAQ, legal. Уникальные title/description, `Organization` + `SoftwareApplication` + `FAQPage` JSON-LD, OG-картинка, sitemap с реальным `lastModified`, подключение Search Console |

Срез 11 добавляется в конец §9, но `metadataBase` и route-группы из среза 1 нужны с самого начала.

### 7.7 Чего не делаем

- **Программные страницы по городам** (`/wake-up/sofia` ×200). Классический thin content: сто страниц, отличающихся одним словом, склеиваются как дубли и тянут вниз весь домен. Десять содержательных страниц лучше двухсот пустых.
- **Публичные профили и рейтинги «для индексации».** Прямо противоречит §0 и минимизации идентичности.
- **`AggregateRating` в разметке**, пока нет реальных отзывов. Выдуманная разметка — прямой путь к ручным санкциям.

### 7.8 Метрики

Покрытие индексации в Search Console (сколько страниц `(app)` просочилось — цель 0), доля страниц с уникальным title, CWV по полевым данным CrUX (не Lighthouse), органические показы по брендовым и небрендовым запросам отдельно.

---

## 8. Открытые вопросы

**8.1 Холодный старт цепочки.** Самого раннего человека дня будить некому — ещё никто не проснулся. Варианты: (а) сид-пул волонтёров/команды на первые недели; (б) первый в дне получает обычный будильник + пуш и сразу становится будильщиком; (в) пары только внутри окна, где кто-то уже бодрствует. Нужно решение — влияет на `matching.ts`.

**8.2 TURN обязателен.** Голый STUN не пробивает symmetric NAT мобильных операторов — 10–20 % звонков просто не соединятся, молча. Нужен провайдер (Cloudflare / Twilio / Metered / свой coturn). Это платная зависимость и внешний аккаунт.

**8.3 Язык интерфейса.** Русский / болгарский / английский — определяет все тексты экранов. Теперь это ещё и SEO-решение: от него зависит письменность заголовков (§7.3) и нужна ли вообще матрица hreflang (§7.4). Один язык — hreflang не нужен, минус заметный кусок работы.

**8.4 Возрастной гейт.** Голосовые звонки незнакомцев с несовершеннолетними — другая правовая категория. Предлагается 18+ по самодекларации при регистрации.

**8.5 Домен и маркетинговый слой.** SEO требует публичных страниц, которых в исходном ТЗ не было — это добавленный scope (срез 11). Нужен домен (`metadataBase` строится от него) и решение, делаем ли лендинг в первой версии или ограничиваемся гигиеной индексации.
...
|---|---|---|---|
| 1 | Скелет + миграции 01–02 | `wakechain/` бутстрап, CI job, route-группы, `metadataBase`, `robots.ts`, `sitemap.ts` | `next build` зелёный, миграции применяются, `(app)` отдаёт `noindex` |
...
| 11 | Маркетинговый слой + SEO | `app/(marketing)/*`, `lib/seo.ts`, `components/seo/*`, `opengraph-image.tsx` | уникальные title/description, JSON-LD проходит Rich Results Test, отрендеренный HTML не пустой в URL Inspection, sitemap отправлен в Search Console |

Каждый срез: миграция → чистая функция + тест → route handler → UI → проверка на телефоне → проверка границы индексации.

Срезы 1–10 не требуют работы по привлечению трафика — там только гигиена (`noindex` там, где надо; SW не ломает рендер). Реальное SEO живёт в срезе 11 и в `metadataBase`/route-группах из среза 1.
...
CRON_SECRET
APP_BASE_URL                   # он же metadataBase — canonical и OG строятся от него
NEXT_PUBLIC_SITE_URL           # публичный базовый URL для sitemap/robots
GOOGLE_SITE_VERIFICATION       # опционально, если верифицируем метатегом
```
